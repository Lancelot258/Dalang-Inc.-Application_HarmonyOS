...
public class OperToolImagFile {
    ...
    static public PixelMap ReadPixelMapFile(String fileName) throws IOException {
        File file=new File(fileName);
        if(!file.exists()) return null;
        int nFileSize= (int) file.length();
        FileInputStream fileInputStream=new FileInputStream(fileName);
    ...
    private static PixelMap ByteToPixelMap(byte[] bytes){
        // 用于 ImageSource的 create(bytes,srcOpts)方法
        ImageSource.SourceOptions srcOpts=new ImageSource.SourceOptions();
        //设置图片原格式也可以用 null
        srcOpts.formatHint="image/jpg";
    ...
    static public void SaveImageToFile(Image image,String fileName,int w,int h)
    {
        image.setWidth(w);
        image.setHeight(h);
    ...
    static public void SavePixelMapToFile(PixelMap pixelMap,String fileName)
    {
    ...
    //将图片转换成byte[]
    static private byte[] GetBytesByImage(Image image) {
    ...
    //将图片转换成byte[]
    static private byte[] GetBytesByPixelMap(PixelMap pm) {
        ImagePacker imagePacker = ImagePacker.create();
        ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream();
        ImagePacker.PackingOptions packingOptions = new ImagePacker.PackingOptions();
        imagePacker.initializePacking(byteArrayOutputStream, packingOptions);
